    ...
    public TenThang(int ten, int diem, int mucchoi) {
        ...
        button.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                setVisible(false);
                String s = "/com/treasurehuntgame/data_top10/top10_" + mucchoi + ".txt";
                String[] tenArr = new String[12];
                int[] diemArr = new int[12];
                ...
                try {
                    ...
                    while (++i <= 10) {
                        ...
                        tenArr[i] = line;
                        line = br.readLine();
                        diemArr[i] = Integer.parseInt(line);
                    }
                    tenArr[11] = cb.getText().trim();
                    if (tenArr[11].isEmpty()) {
                        tenArr[11] = "no name";
                    }
                    diemArr[11] = diem;
                    int o;
                    ...
                    for (i = 10; i > o; i--) {
                        tenArr[i] = tenArr[i - 1];
                        diemArr[i] = diemArr[i - 1];
                    }
                    tenArr[o] = tenArr[11];
                    diemArr[o] = diem;

                    try (BufferedWriter bw = new BufferedWriter(new FileWriter(getClass().getResource(s).getPath()))) {
                        for (i = 1; i <= 10; i++) {
                            bw.write(tenArr[i] + "\n");
                            bw.write(diemArr[i] + "\n");
                        }
